...
if [ ! -f N4/$subj.nii.gz ];then 
  ...
  if [ ! -f segmentations/${subj}_brain_mask.nii.gz ];then
    #brain extract
    bet N4/${subj}_rescaled.nii.gz segmentations/${subj}_brain.nii.gz -R -f 0.1 -m 
    rm segmentations/${subj}_brain.nii.gz
  fi

  #bias correct
  run N4BiasFieldCorrection -d 3 -i N4/${subj}_rescaled.nii.gz -x segmentations/${subj}_brain_mask.nii.gz -o "[N4/${subj}_corr.nii.gz,bias/$subj.nii.gz]" -c "[50x50x50,0.001]" -s 2 -b "[100,3]" -t "[0.15,0.01,200]"
